    ...
    if (isset($_POST['data_conta']))
    {
        ...
        if ($conn)
        {
            ...
            if ($_SESSION['opc'] == 1)
            {
                ...
                {
                    $_SESSION['erro'] = 'O valor informado no campo Total não é igual a soma dos valores informados nos campos Valor consumo, Valor da multa, Valor do juros, Religação e  Emissão 2° via.';
                    header('Location: ' . $_SESSION['pagina']);
                    exit;
                }
